...
class ContentProcessor:
    ...
    def __init__(self,config):
        ...
        """
         ...
         self.summarier_model=config.rag.summarizer_model
         self.chunker_model=config.rag.chunker_model

    def summarier_images(self,images: List[str])->List[str]:
        """
        ...
        summary_chain=prompt | self.summarier_model | StrOutputParser()
    ...
    def replace_occurences(self,text:str,target:str,replacements:List[str])->str:
    ...
    def chunk_documents(self,formatted_document:str)->List[str]:
        """
        Split the document into semantic chunks.

        Args:
        formatted_document: Formatted document text
        model: AureChatOpenAI model instance (will create one if not provided)
        ...
        chunking_response=self.chunker_model.invoke(formatted_chunking_prompt)
        
        return self._split_text_by_llm_suggesions(chunked_text,chunking_response)

    def _split_text_by_llm_suggesions(self,chunked_text:str,llm_response: str)->List[str]:
        """
